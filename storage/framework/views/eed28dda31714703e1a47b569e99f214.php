        ...
        <div class="row">
            <div class="col-md-3">
                <div class="text-center p-3">
                    ...
                    <h4 class="text-primary">Rp <?php echo e(number_format(\App\Models\Product::avg('harga'), 0, ',', '.')); ?></h4>
                </div>
            </div>
            <div class="col-md-3">
                <div class="text-center p-3">
                    <h6 class="text-muted">Total Stok Barang</h6>
                    <h4 class="text-success"><?php echo e(number_format(\App\Models\Product::sum('stok'), 0, ',', '.')); ?> Unit</h4>
                </div>
            </div>
            <div class="col-md-3">
                <div class="text-center p-3">
                    <h6 class="text-muted">Produk Stok Rendah</h6>
                    <h4 class="text-warning"><?php echo e(\App\Models\Product::where('stok', '<', 10)->count()); ?> Produk</h4>
                </div>
            </div>
            <div class="col-md-3">
                <div class="text-center p-3">
                    <h6 class="text-muted">Produk Habis</h6>
                    <h4 class="text-danger"><?php echo e(\App\Models\Product::where('stok', '=', 0)->count()); ?> Produk</h4>
                </div>
            </div>
        </div>
    </div>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        // Data dari backend
        const categoryData = <?php echo json_encode(\App\Models\Category::withCount('products')->get(), 15, 512) ?>;
        const lowStockData = <?php echo json_encode(\App\Models\Product::orderBy('stok', 'asc')->limit(10)->get(), 512) ?>;
        const supplierData = <?php echo json_encode(\App\Models\Supplier::withCount('products')->get(), 15, 512) ?>;
        
        // Query untuk nilai stok per kategori
        const stockValueData = <?php echo json_encode(\App\Models\Category::with('products')->get()->map(function($category) {
            return [
                'name' => $category->nama_kategori, 'value' => $category->products->sum(function($product) { return $product->harga * $product->stok; })
            ];
        }), 512) ?>;
